...
class LiveCryptoDashboard:
    ...
    def run_dashboard(self):
        st.set_page_config("Market Pulse – Quant Analytics", layout="wide")
        st.title("📈 Market Pulse: Real-Time Quant Analytics")
        ...
        with col1:
            fig_z = go.Figure()
            fig_z.add_trace(go.Scatter(x=df["time"], y=z, name="Z-score"))
            st.plotly_chart(fig_z, use_container_width=True)

        with col2:
            fig_c = go.Figure()
            fig_c.add_trace(go.Scatter(x=df["time"], y=corr, name="Correlation"))
